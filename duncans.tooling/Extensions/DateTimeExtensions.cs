...
namespace duncans
{
    public static class DateTimeExtensions
    {
        ...
        public static DateTime NetworkDays(this DateTime value, int daysFromNow, List<DateTime> holidayDates, WorkWeekModel workweek)
        {
            DateTime result = value;
            ...
            while (daysFromNow > 0)
            {
                ...
                if (!(result.DayOfWeek.IsWorkingDay(workweek) == false || isHoliday == true))
                {
                    daysFromNow--;
                }
            }
        ...
        /// <summary>
        /// See if the day of the week is a working day.
        /// </summary>
        /// <param name="value"></param>
        /// <param name="workweek"></param>
        /// <returns></returns>
        public static bool IsWorkingDay(this DayOfWeek value, WorkWeekModel workweek)
        {
            bool result = false;
            DayOfWeek currentDayOfWeek = workweek.FirstWorkingDayOfWeek;
            int dayOffset = (int)currentDayOfWeek; // Sunday = 0, Saturday = 6

            for (int i = 0; i < workweek.WorkingDay.Length; i++)
            {
                if (currentDayOfWeek == value)
                {
                    result = workweek.WorkingDay[i];
                    break;
                }
                ...
                {
                    currentDayOfWeek = (DayOfWeek)i + dayOffset + 1;
                }
            }
        ...
        {
            return FromEpoch(Convert.ToInt64(epochTime.RoundDown(0)));
        }
