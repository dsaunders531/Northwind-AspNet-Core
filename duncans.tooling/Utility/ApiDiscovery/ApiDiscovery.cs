// <copyright file="ApiDiscovery.cs" company="Duncan Saunders">
...
namespace duncans.ApiDiscovery
{
    ...
    public sealed class ApiDiscovery
    {
        ...
        public ApiControllersViewModel Discover()
        {
            ...
            for (long i = 0; i < assemblyTypes.LongLength; i++)
            {
                ...
                if (assemblyType.BaseType == typeof(Microsoft.AspNetCore.Mvc.Controller)
                    || assemblyType.BaseType?.BaseType == typeof(Microsoft.AspNetCore.Mvc.Controller)
                    || assemblyType.BaseType == typeof(Microsoft.AspNetCore.Mvc.ControllerBase)
                    || assemblyType.BaseType?.BaseType == typeof(Microsoft.AspNetCore.Mvc.ControllerBase))
                {
        ...
        private bool HasApiAttrs(System.Type assemblyType, out string attrName, out string route)
        {
            ...
            bool doNotDiscover = false;

            try
            {
                Attribute attribute = assemblyType.GetCustomAttribute(typeof(DoNotDiscoverAttribute));
                doNotDiscover = attribute != null;
            }
            catch (Exception)
            {
        ...
        private bool FindApiMethod(MethodInfo method, string controllerRoute, out ApiMethod apiMethod, out string routeInfo)
        {
            ...
            try
            {
                methodAttr = method.GetCustomAttribute(typeof(DoNotDiscoverAttribute));
                doNotDiscover = methodAttr != null;
                methodAttr = null;
            }
            catch (Exception)
            {
