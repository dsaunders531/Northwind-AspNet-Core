// <copyright file="FileCompression.cs" company="Duncan Saunders">
...
namespace duncans.Utility
{
    ...
    {
        private const int FileSize = 4096; // bytes in the stream
        ...
        private void GetFileFromZipStream(Stream s, string outFile)
        {
            using (FileStream outF = File.Create(outFile))
            {
                ...
                int numread = s.Read(b, 0, FileSize);
                while (numread != 0)
                {
                    outF.Write(b, 0, numread);
                    numread = s.Read(b, 0, FileSize);
                }
            }
        }
    }
