// <copyright file="DbClient.cs" company="Duncan Saunders">
                ...
                if (row[column] != null)
                {
                    ...
                    foreach (PropertyInfo item in outputType.GetProperties())
                    {
                        if (item.Name.ToLower() == column.ColumnName.ToLower())
                        {
                            ...
                            {
                                // Handle the dbnull conversion more gracefully
                                if (e.Message.Contains("DBNull")
                                    && (e.Message.Contains("Nullable") || e.Message.Contains("String")))
                                {
                                    item.SetValue(inputObject, null);
                                }
                                else
                                {
                                    throw;
