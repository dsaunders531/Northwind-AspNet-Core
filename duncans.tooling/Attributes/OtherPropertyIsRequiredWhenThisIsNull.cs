// <copyright file="OtherPropertyIsRequiredWhenThisIsNull.cs" company="Duncan Saunders">
...
{
    /// <summary>
    /// Validator to use when another property must be populated if this one is not populated.
    /// The other property must have a value other than its default.
    /// </summary>
    public class OtherPropertyIsRequiredWhenThisIsNull : ValidationAttribute, IModelValidator
    {
                    ...
                    result = new List<ModelValidationResult>()
                            {
                                new ModelValidationResult(
                                                            ...
                                                            string.Format(
                                                                        "{0} must have a value when {1} does not have a value.",
                                                                        otherDisplayName,
                                                                        thisDisplayName))
                            };
                }
